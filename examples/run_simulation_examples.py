...
Seven examples, each a kickoff gate:
  ...
  * **F — external CFD** (§M3): ``cfd_external_flow_submit`` builds a 2-D laminar flat
    ...
    the U^1.5 law.
  * **G — modal** (§M6): a steel cantilever meshed with **2nd-order tets** and solved by
...
def example_modal(w, log):
    ...
    log("### Example G — fem_modal (CalculiX, 2nd-order tets) vs beam_modal oracle  (§M6)")
...
def _plot_modal(outdir):
    ...
    fig.suptitle("Example G — CalculiX modal vs Euler-Bernoulli beam oracle  (§M6)",
                 fontsize=11, weight="bold")
    fig.tight_layout(rect=(0, 0, 1, 0.95))
...
def make_plots(outdir):
    """Generate the seven result figures into ``outdir``; skip a panel when its solver

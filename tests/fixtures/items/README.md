...
- **`items.json`** — a well-formed `driftpin.items/1` registry: two items
...
- **`manifest_with_items.json`** — a manifest whose components reference items by
  ...
  **item-reference form**. `driftpin.items.validate_manifest_refs` resolves these
  against `items.json`; a dangling reference is caught.

The artifact files the items point at are illustrative paths (C1 is pure
data/text identity — it does not build geometry). See `driftpin/items.py` for the

...
| The FreeCAD **Render workbench** add-on renderers other than POV-Ray (appleseed, LuxCore, Cycles, OSPRay, pbrt) — 5 tests in `test_render_photoreal.py` | every lane | hand-fetched builds of hundreds of MB each; the heavy image bundles the add-on + POV-Ray only (#421) | the add-on path itself (import, scene export, material cards, `render_photoreal_submit` → `render_job`) runs through POV-Ray on both heavy image lanes, and the preflight fails if the add-on or POV-Ray stops resolving. Per-renderer: by hand, `scripts/install-renderers.sh` + `tests/test_render_photoreal.py` |

...
* **Catalog (offline, deterministic).** Resolve a designation against the curated
  off-the-shelf corpus in :mod:`driftpin.analysis.standards`: is this a stocked
...
FreeCAD-free on purpose, like :mod:`driftpin.inspection` — everything here is string
...
def designate_oring(inner_diameter_mm, cross_section_mm, compound=None):
    ...
    Sizes come from :func:`driftpin.analysis.standards.as568_lookup`. When the size
...
#
# Whether a BOM row is a purchased standard part is EXACT for anything DriftPin
...
#
# The corpus lives in driftpin.analysis.standards (catalog.json) next to the other
...
def catalog_nearest(standard, size, length=None, grade=None):
    ...
    :class:`driftpin.analysis.standards.StandardNotFound` for a standard the catalog
...
def designation_check(rows):
    ...
    complete?}``. ``part_class == 'purchased'`` is the EXACT signal (a DriftPin
    ...
    return {
        ...
        "basis": "exact for DriftPin-generated parts (stamped part_class); a name "

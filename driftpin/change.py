...
on top of the C1 item model (driftpin/items.py, #140) and the C2 revision +
lifecycle state machine (driftpin/lifecycle.py, #141).
...
or verify a baseline, exactly like driftpin/items.py and driftpin/lifecycle.py.
...
eco (schema "driftpin.eco/1"):

    { "schema": "driftpin.eco/1",
      "id": "ECO-0001",
...
baseline (schema "driftpin.baseline/1"):

    { "schema": "driftpin.baseline/1",
      "label": "v1.0",
      "items": {
...
ECO_SCHEMA = "driftpin.eco/1"
BASELINE_SCHEMA = "driftpin.baseline/1"

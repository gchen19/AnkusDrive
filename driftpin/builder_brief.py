...
DriftPin's multi-agent contract (docs/MULTI_AGENT.md) is a partition-and-merge
...
Code subagent, a Cursor task, a human) can consume with the full DriftPin tool
...
def builder_brief_text(brief):
    ...
    this into a subagent prompt; the subagent then drives the full DriftPin tool
...
def brief_from_slice(coordinator_brief, cid, *, output=None):
    ...
    owns a private slice format — it emits a `driftpin.builder_brief/1`."""

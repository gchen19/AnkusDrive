...
DriftPin's own process never imports KrakenOS; the worker invokes this file in a
...
arm's-length isolation DriftPin already uses for the GPL Elmer/OpenFOAM binaries, so
the copyleft of KrakenOS does not reach into DriftPin's MIT/permissive code.
...
"solid_trace": trace a ray bundle non-sequentially through ONE STL solid that has a
    refractive index. Args:
        stl_path       : path to the solid mesh (DriftPin exports this from the model)

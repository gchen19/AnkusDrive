"""``driftpin setup`` — the interactive provisioner tying the three onboarding
...
into **this interpreter** (the one that runs ``driftpin mcp`` — the discovery
contract requires that); the source-build / system solvers (OpenFOAM, Elmer,
YADE, openEMS, preCICE) are never installed by this command — ``driftpin
...
# The pip-wheel extras a plain `pip install driftpin[<extra>]` provisions —
...
def _launcher() -> list:
    ...
    ``driftpin`` entry-point script when it resolves (absolute path — GUI hosts
    don't inherit shell PATH), else ``<this python> -m driftpin``."""
    exe = shutil.which("driftpin")
    if not exe:
        # entry-point script next to the current interpreter (venv/pipx layout)
        cand = os.path.join(os.path.dirname(sys.executable),
                            "driftpin.exe" if os.name == "nt" else "driftpin")
        if os.path.isfile(cand):
            exe = cand
    if exe:
        return [os.path.abspath(exe), "mcp"]
    return [sys.executable, "-m", "driftpin", "mcp"]
...
def mcp_config_text() -> str:
    ...
    argv = _launcher()
    snippet = {"mcpServers": {"driftpin": {"command": argv[0], "args": argv[1:]}}}
    lines = [
        ...
        f"  claude mcp add driftpin -- {' '.join(argv)}",
        "",
        "Zero-install variant (once driftpin is on PyPI):",
        '  "command": "uvx", "args": ["driftpin", "mcp"]',
    ]
    return "\n".join(lines)
...
def _install_extras(extras: list) -> None:
    ...
    for extra in extras:
        spec = f"{repo}[{extra}]" if editable else f"driftpin[{extra}]"
...
def run_setup(assume_yes: bool = False, extras: list | None = None,
    ...
    to_persist = {}
    if fc_path:
        to_persist["DRIFTPIN_FREECADCMD"] = fc_path
    ...
    print("preCICE) are NOT installed by this command — run `driftpin doctor` for")

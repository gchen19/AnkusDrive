...
#226 made a quantitative spec an object that rides on the part (``DP_Performance``)
...
role ``driftpin.gates.modal`` plays for ``min_first_mode_hz``.
...
stamps its verdict onto the part (``DP_PerformanceVerdict``); every gate reads that
...
RECORD_SCHEMA = "driftpin.performance_verdict/1"
#: Schema tag for the gate block this module produces.
SCHEMA = "driftpin.performance_gate/1"
...
def evaluate(contract, record, signature=None, component=None):
    ...
    Args:
      contract:  the ``DP_Performance`` bag (``{"requirements": [...]}``), or None/{}
                 for a part that declares nothing.
      record:    the ``DP_PerformanceVerdict`` bag written by ``verify_performance``,
...
def brief_problems(slice_):
    ...
    ``driftpin.builder_brief.validate_builder_brief``'s style."""

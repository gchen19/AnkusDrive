#!/usr/bin/env bash
#
# build-renderers.sh — build the three source-only photoreal renderers DriftPin's
...
#   writes a wrapper to $BINDIR named exactly like the binary DriftPin's _RENDERERS
#   registry looks for (driftpin/worker.py): `pbrt`, `cycles`, `ospStudio`. The
...
#   (`python -m driftpin mcp`); a wrapper in a system PATH dir covers that.
#
# USAGE
...
#   BUILD_ROOT  build/scratch dir      (default: ${TMPDIR:-/tmp}/driftpin-renderer-build)
...
BUILD_ROOT="${BUILD_ROOT:-${TMPDIR:-/tmp}/driftpin-renderer-build}"
...
write_exec_wrapper() {  # wrapper_path real_binary [libdirs_abs]
  ...
  if [ -n "$libs" ]; then
    cat > "$wrapper" <<EOF
#!/bin/sh
# DriftPin renderer wrapper — generated by scripts/build-renderers.sh.
...
main() {
  ...
  case ":$PATH:" in
    *":$BINDIR:"*) : ;;
    *) warn "$BINDIR is not on PATH — add it in the env that launches 'python -m driftpin mcp'" ;;
  esac
}

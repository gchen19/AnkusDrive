...
(out-of-band→0, static-overload). See [`ADVANCED_TOYS_PLAN.md`](archive/ADVANCED_TOYS_PLAN.md)
for the per-family coverage map and a [figure gallery](archive/ADVANCED_TOYS_PLAN.md#figures)
of the laws (generated by [`examples/plot_advanced_toys.py`](../examples/plot_advanced_toys.py)).

**Status: every family below has shipped.** This doc was written while they were
proposals, and the per-section tier tags record what each one *cost* to build, not
whether it exists:

- ✅ **anchor** — the FEM family, already callable when this doc was written.
- 🟡 **P0** — pure Python, zero new deps.
- 🟠 **P1** — one external CLI / in-house pipeline.
- 🔴 **P2** — heavy external solver, behind the async submit→poll job pattern
  (`*_submit` + `job_status`/`job_result`).

Treat the sections as a *verification* catalog: the toy problem and its known answer
are the durable part. `SIMULATION_TOOLS.md` carries the authoritative signatures, and
where a P2 family's example predates the async split its call is marked below.
...
tolerance_stackup(
  chain=[
    ...
    {"name":"shoulder_to_shoulder","nominal":30.0, "tol":0.05, "direction":-1},
...
The chain is a *gap*: the shoulder and shim subtract from the bore depth, so each
carries `direction:-1` and the nominal closes at 50 − 30 − 2 = 18.0. Without the
direction keys every link adds and the tool returns 82.0 — this exact chain is
gated by `test_doc_example_with_directions` in [`tests/test_tolerance.py`](../tests/test_tolerance.py).
...
```text
# shipped as cfd_internal_flow_submit — submit→poll, results via job_result
cfd_internal_flow_submit(model="body_1",
...
```text
# Sketch of the intent. Shipped as two tools with a different shape:
#   mechanism_kinematics(...)          — closed-form, synchronous
#   mechanism_simulate_submit(links, drivers, duration_s, dt_s, gravity,
#                             obstacles, base, loop_closures, gears)
# takes `links` rather than assembly+joints; see SIMULATION_TOOLS.md §8 for the
# authoritative signature.
mechanism_simulate_submit(
  links=[...four-bar link definitions...],
...
3. **Pure-Python families need no FreeCAD** — their toys run in milliseconds and
   belong in `tests/test_*.py` wired into `tests/run_all.sh` — as
   [`tests/test_tolerance.py`](../tests/test_tolerance.py) and its siblings now do.

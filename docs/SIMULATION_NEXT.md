...
| Explicit impact dynamics (**#311**) | partial | an exact anchor exists | L — horizon; pick up only on a concrete need. **The only unbuilt item left in this table**, and tracked in #311 rather than here. Free-surface flow (`interFoam`) has since shipped as the fallback backend of `molding_fill_submit` (`worker.py`, VOF melt+air cavity fill) — see [`MOLDING_FILL_SOLVER.md`](MOLDING_FILL_SOLVER.md); RF/wave EM shipped as full-wave openEMS, the row above. |

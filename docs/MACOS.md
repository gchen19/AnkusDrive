...
| Transient/radiation thermal | Elmer | ⚠️ **no prebuilt macOS binaries exist** — no Homebrew formula, no conda-forge package (older hints claiming one were wrong). `scripts/install-solvers.sh elmer` source-builds a pinned release (CMake + gfortran, ~minutes) into the provisioner dir, auto-discovered (#384) |

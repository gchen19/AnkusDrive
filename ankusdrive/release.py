...
FreeCAD-free on purpose, exactly like :mod:`ankusdrive.drawing_gate` and
:mod:`ankusdrive.inspection`: manifest composition, checksums, the releasable/
...
is a :func:`ankusdrive.analysis.cost.cost_estimate` rollup, and it inherits that
...
def identity(item_id, record, *, eco=None):
    ...
    Reads it off the item registry record (:mod:`ankusdrive.items`), which is the
...
def resolve_kinds(kinds=None, *, rfq=False):
    ...
    * ``inspection`` implies a drawing kind. The inspection deliverable is a
      *ballooned print* plus its plan; asking for the plan without the print it
      ...
      find them on. ``drawing_svg`` is the implied one (it is the format AnkusDrive
...
def canonical_bytes(name, data, *, product=None):
    ...
    * everything else — returned unchanged. The drawing SVG is composed by AnkusDrive
...
def bom_csv(rows, *, part=None, rev=None, eco=None):
    ...
    :func:`ankusdrive.inspection.fai_csv` applies. Rows are emitted in the order
...
def quantity_break_table(volume_mm3, material, breaks, *, process="cnc", **kw):
    ...
    :func:`ankusdrive.analysis.cost.cost_estimate` once per quantity and reports the
    ...
    from ankusdrive.analysis import cost as _cost
    ...
    return {
        ...
        "band_pct": 100.0,
        "basis": "ankusdrive cost_estimate rollup (exact material cost over an "

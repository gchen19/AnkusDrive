...
real solid (``AD_TrueValue``). What it cannot do is judge the *set*. A drawing that
...
the same failure mode the geometry-realizes-declaration gate (:mod:`ankusdrive.realize`)
...
def check_completeness(features, dims, process, *, datums_declared=False, notes=None):
    ...
    (``AD_FaceRoles``); when true, a location dimension that is not measured *from*
...
# :mod:`ankusdrive.analysis.tolerance` so there is one source of truth for the bands.

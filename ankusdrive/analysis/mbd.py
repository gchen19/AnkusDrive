...
handled by ``ankusdrive.solvers``; this runs only once a solver resolved.
...
NOT computed here — they are the closed-form gate in ``ankusdrive.analysis.kinematics``
...
def run_mbd(spec: dict, duration_s: float = 1.0, dt_s: float = 1.0 / 240.0,
    ...
    PyBullet is missing (gate on ankusdrive.solvers.require_solver)."""
    try:
        p, cid = _connect()

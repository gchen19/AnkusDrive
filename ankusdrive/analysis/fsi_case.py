...
OpenFOAM ↔ CalculiX stack (the executable twin of ``ankusdrive/analysis/fsi.py``).
...
The stack is resolved through ``ankusdrive.solvers`` (the ``fsi`` family gates on
...
def _participant_script(setup_lines: list, exec_line: str) -> str:
    ...
    lines = ["echo $$ > .ankusdrive-participant.pid"] + [l for l in setup_lines if l]
    return "\n".join(lines) + f"\nexec {exec_line}"
    ...
    if solvers.runs_in_substrate():
        try:
            ...
            subprocess.run(solvers.bash_argv(
                "kill -TERM $(cat .ankusdrive-participant.pid 2>/dev/null) "
                "2>/dev/null; sleep 2; "
                "kill -KILL $(cat .ankusdrive-participant.pid 2>/dev/null) "

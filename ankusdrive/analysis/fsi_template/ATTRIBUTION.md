...
that `ankusdrive/analysis/fsi_case.py` substitutes physics into and runs as the real
coupled FSI solve (issue #91).

The two heavy solvers run **out-of-process** (the arm's-length boundary AnkusDrive
already uses for OpenFOAM/Elmer); preCICE (LGPL-3.0) is never imported into
AnkusDrive's permissive code. Only these data files are vendored — no LGPL source is

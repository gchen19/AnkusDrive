...
Exploring a design space in AnkusDrive has always been possible and never been a
...
existing AnkusDrive tool and a dotted path into its result, the same mapping the
...
def validate_responses(responses) -> list:
    ...
    Shape (the performance-contract mapping, deliberately)::
        ...
        {"name": "dp",
         "tool": "cfd_pipe_flow",              # any AnkusDrive tool
    ...
    for resp in responses:
        ...
        if not resp.get("tool"):
            raise ValueError(f"response {name!r} needs a 'tool' — the AnkusDrive tool that "

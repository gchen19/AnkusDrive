...
cast irons …). This module is **the FCMat → AnkusDrive-schema mapping source of
...
def _candidate_dirs(extra: str | None = None) -> list[Path]:
    ...
    env = os.environ.get("ANKUSDRIVE_FCMAT_DIR")
    if env:
...
def load_fcmat_cards(directory: str | None = None) -> list[dict]:
    ...
    Looks in `directory`, then $ANKUSDRIVE_FCMAT_DIR, then FreeCAD's resource dir

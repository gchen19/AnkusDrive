...
AnkusDrive's own process never imports YADE; the worker invokes the ``yade``
...
same arm's-length isolation AnkusDrive already uses for the GPL Elmer/OpenFOAM
binaries and the KrakenOS optics runner, so the copyleft of YADE does not reach
into AnkusDrive's MIT/permissive code.

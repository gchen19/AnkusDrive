...
A *part recipe* (issue #136, :mod:`ankusdrive.recipes`) is a named build function
...
onto an existing host body in new contexts. It is AnkusDrive's PowerCopy/UDF: the
...
Two declaration kinds make up a template's input surface:
  ...
  2. SCALAR inputs (reuse :class:`ankusdrive.recipes.InputSpec`) — the published
...
The frozen contract (what a consumer pins to):
  ...
  * Per-template SCHEMA form (:func:`feature_schema`):
        ...
        { "schema": "ankusdrive.feature_template/1",
...
from ankusdrive import recipes
from ankusdrive.recipes import InputSpec  # re-export: templates declare scalars with it

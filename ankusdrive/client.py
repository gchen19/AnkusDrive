"""
AnkusDrive host-side client — spawns `freecadcmd worker.py`, talks to it over
...
# path. Resolution: ANKUSDRIVE_FREECADCMD env → PATH (any name) → these globbed candidates.
...
def _resolve_freecadcmd():
    # env override -> config file (~/.config/ankusdrive/config.toml `freecadcmd`;
    # the persistent layer MCP hosts' minimal env can't strip, issue #199)
    from ankusdrive import config as _config
    if explicit := _config.get("ANKUSDRIVE_FREECADCMD"):
        return explicit

...
as ankusdrive/recipes.py does — so table parsing, the column split, door validation,
...
is sourced from the ISO standards corpus (ankusdrive/analysis/standards) so a built
catalog part matches the standard table value by construction.

------------------------------------------------------------------------------
The family-table contract (schema "ankusdrive.family/1"):
...
from ankusdrive import recipes as _recipes
...
def _build_ball_bearing(p, call):
    ...
    the ISO standards corpus (ankusdrive/analysis/standards) by designation — so a

"""
AnkusDrive worker — runs inside `freecadcmd`, reads JSON requests on stdin,
...
from ankusdrive.props import (  # noqa: E402
    ...
    prop_get as _prop_get,
    prop_set as _prop_set,
)
...
def _h_oring_groove(p):
    ...
    if result["oring"].get("designation"):
        _stamp_str_prop(out, "AD_ORing", json.dumps(result["oring"], sort_keys=True))
    result["handle"] = h
...
def _resolve_port_face(handle, shape, ref, label):
    ...
    face-role NAME or ROLE string (resolved through AD_FaceRoles via the stored
...
def _h_run_script(p):
    ...
    if _SCRIPT_GLOBALS is None:
        _SCRIPT_GLOBALS = {
            ...
            "_resolve": _resolve,
            "_handles": _handles,
            "__name__": "__ankusdrive_script__",
        }
    code = p["code"]
...
#
# Frames persist in the component file as a JSON property bag (AD_Interfaces) on
...
def _read_interfaces(obj):
    ...
    if _prop_has(base, _IFACE_PROP):
        try:
            return _json.loads(_prop_get(base, _IFACE_PROP) or "{}")
        except Exception:
            return {}
    return {}
...
def _h_publish_interface(p):
    ...
    ifaces[name] = frame
    if not _prop_has(base, _IFACE_PROP):
        base.addProperty("App::PropertyString", _IFACE_PROP, "AnkusDrive",
                         "published interface frames (JSON)")
    _prop_set(base, _IFACE_PROP, _json.dumps(ifaces))
...
# property bag, mirroring publish_interface's AD_Interfaces exactly. The stored
...
def _read_face_roles(obj):
    ...
    import json as _json
    base = _shaped_top(obj)
    if _prop_has(base, _FACEROLE_PROP):
        try:
            return _json.loads(_prop_get(base, _FACEROLE_PROP) or "{}")
        except Exception:
            return {}
    return {}
...
def _h_annotate_face(p):
    ...
    roles[name] = entry
    base = _shaped_top(obj)
    if not _prop_has(base, _FACEROLE_PROP):
        base.addProperty("App::PropertyString", _FACEROLE_PROP, "AnkusDrive",
                         "semantic face roles (JSON)")
    _prop_set(base, _FACEROLE_PROP, _json.dumps(roles))
    base.Document.recompute()
...
# presence) and persists as a JSON property bag (AD_Intent), like AD_Interfaces /
# AD_FaceRoles. verify_intent never raises on a failing invariant: a failure
# becomes a {passed: False} row so the gate is safe to run in a loop.

_INTENT_PROP = "AD_Intent"
...
def _read_intent(obj):
    ...
    import json as _json
    base = _shaped_top(obj)
    if _prop_has(base, _INTENT_PROP):
        try:
            return _json.loads(_prop_get(base, _INTENT_PROP) or "{}")
        except Exception:
            return {}
    return {}
...
def _h_declare_intent(p):
    ...
    a JSON property bag (AD_Intent); one contract per part, re-declaring replaces.
    ...
    base = _shaped_top(obj)
    if not _prop_has(base, _INTENT_PROP):
        base.addProperty("App::PropertyString", _INTENT_PROP, "AnkusDrive",
                         "declared functional intent (JSON)")
    _prop_set(base, _INTENT_PROP, _json.dumps(contract))
    base.Document.recompute()
    return {"handle": handle, "contract": contract}


_PERF_PROP = "AD_Performance"
...
# #226 exists to prevent. See ankusdrive/gates/performance.py for the policy that makes
# a synchronously-answering gate legitimate over an asynchronously-produced verdict.
_PERF_VERDICT_PROP = "AD_PerformanceVerdict"
...
def _read_performance(obj):
    ...
    import json as _json
    base = _shaped_top(obj)
    if _prop_has(base, _PERF_PROP):
        try:
            return _json.loads(_prop_get(base, _PERF_PROP) or "{}")
        except Exception:
            return {}
    return {}
...
def _read_performance_verdict(obj):
    ...
    base = _shaped_top(obj)
    if _prop_has(base, _PERF_VERDICT_PROP):
        try:
            rec = _json.loads(_prop_get(base, _PERF_VERDICT_PROP) or "null")
        except Exception:
            return None
...
def _write_performance_verdict(obj, record):
    ...
    import json as _json
    base = _shaped_top(obj)
    if not _prop_has(base, _PERF_VERDICT_PROP):
        base.addProperty("App::PropertyString", _PERF_VERDICT_PROP, "AnkusDrive",
                         "last recorded performance verdict (JSON)")
    _prop_set(base, _PERF_VERDICT_PROP, _json.dumps(record))
    base.Document.recompute()
    return record
...
def _performance_gate_block(obj, component=None):
    ...
    signature, judged by the pure ankusdrive.gates.performance. Never raises."""
...
def _evaluate_measurement(req, payload, tier, metric=None, band=None):
    ...
    from ankusdrive.analysis import performance as _pf
...
def _h_declare_performance(p):
    ...
    Persists in the .FCStd as a JSON property bag (AD_Performance); one contract per
    ...
    base = _shaped_top(obj)
    if not _prop_has(base, _PERF_PROP):
        base.addProperty("App::PropertyString", _PERF_PROP, "AnkusDrive",
                         "declared performance contract (JSON)")
    _prop_set(base, _PERF_PROP, _json.dumps(contract))
    base.Document.recompute()
    return {"handle": handle, "contract": contract,
...
def _h_verify_performance(p):
    ...
    Every verdict is also RECORDED on the part (AD_PerformanceVerdict), stamped with a
...
def _h_study_submit(p):
    ...
    `"$handle"` is the part it built. Because `tool` is any AnkusDrive tool, a response can
    ...
    from ankusdrive import jobs
    from ankusdrive.analysis import study as _st
...
def _refuse_unbuildable_handle(where, conditions, has_geometry):
    ...
    queue (ankusdrive/mainthread.py) is what lifted that, by giving a job thread a way to
...
def _h_optimize_submit(p):
    ...
    through the work queue in ankusdrive/mainthread.py, because FreeCAD's document API is
    ...
    from ankusdrive import recipes as _rc
    from ankusdrive.analysis import optimize as _opt
    from ankusdrive.analysis import performance as _pf
    ...
    # queue; see ankusdrive/mainthread.py for why the client's own polling is what
...
# ankusdrive.builder_brief/1 slice). It runs exactly the three checks merge_assembly
...
# pure, FreeCAD-free ankusdrive.builder_brief.evaluate_contract, so it is testable
...
def _h_component_contract_check(p):
    ...
    handle: the component's shaped object.
    brief:  a ankusdrive.builder_brief/1 slice (see ankusdrive.builder_brief). Only
    ...
    from ankusdrive import builder_brief as _bb
    handle = p["handle"]
...
def _interfaces_of_file(path):
    ...
    try:
        for o in doc.Objects:
            if _prop_has(o, _IFACE_PROP):
                import json as _json
                try:
                    return _json.loads(_prop_get(o, _IFACE_PROP) or "{}")
                except Exception:
                    return {}
        return {}
    finally:
...
_MANIFEST_SCHEMA = "ankusdrive.manifest/1"
...
def _validate_manifest(man):
    ...
    if mech is not None:
        from ankusdrive import mechanism as _mech
...
# moment it is generated — AD_Designation (human-readable, visible in the property
# editor) plus AD_OrderableSpec (the full card as JSON) plus AD_PartClass. It lives
...
# from. Same idempotent add-then-set shape as AD_Balloon / AD_TitleBlock.
...
def _stamp_orderable(obj, card):
    ...
    if not card or not card.get("designation"):
        return
    _stamp_str_prop(obj, "AD_Designation", card["designation"])
    _stamp_str_prop(obj, "AD_PartClass", "purchased")
    _stamp_str_prop(obj, "AD_OrderableSpec", json.dumps(card, sort_keys=True))


def _orderable_of(obj):
    """Read the designation card back off an object, or {} when it carries none.

    Falls back to the bare AD_Designation string when the JSON blob is missing or
    ...
    raw = str(_prop_get(obj, "AD_OrderableSpec", "") or "")
    if raw:
        try:
            card = json.loads(raw)
            ...
            pass
    text = str(_prop_get(obj, "AD_Designation", "") or "")
    if text:
...
def _consumable_of(obj):
    ...
    BOM at all."""
    raw = str(_prop_get(obj, "AD_ORing", "") or "")
    if not raw:
        return {}
    try:
        card = json.loads(raw)
            ...
            if orderable:
                ...
                counts[key].update({
                    ...
                    "part_class": "purchased" if card.get("designation") else
                                  str(_prop_get(base, "AD_PartClass", "") or ""),
                })
        else:
            row["count"] += 1
...
def _h_designation_check(p):
    ...
    order it (no material grade). Purchased-ness is EXACT for AnkusDrive-generated
...
def _resolve_fit_band(chk):
    ...
    if "fit_class" in chk:
        from ankusdrive.analysis import tolerance as _tol
...
def _gate_press_fit(by_name, links_by_inst, chk):
    ...
    from ankusdrive.analysis import tolerance as _tol
    from ankusdrive.analysis import machine_elements as _me
    try:
        if "basic_size_mm" not in chk:
...
# geometry implements the claim. See ankusdrive/realize.py and
...
# ankusdrive/iface_registry.py; this thin gate is the only geometry-touching step:
...
def _gate_interface_conformance(by_name, links_by_inst, chk):
    ...
    from ankusdrive import iface_registry as _ir
    part = by_name.get(links_by_inst.get(chk["part"], chk["part"]))
    if part is None:
...
# not a statement about the part. ankusdrive.gates.modal owns that classification.
...
def _ccx_absent_or_failed():
    ...
    from ankusdrive.gates import modal as _modal
    try:
...
def _first_mode_requirement(assembly_handle, spec, shapes, links_by_inst):
    ...
    marker, built by ankusdrive.gates.modal so every consumer classifies it the
...
def _solve_fused_first_mode(fused, fix_faces, material, spec, assembly_handle):
    ...
    App.setActiveDocument(doc.Name)
    workdir = _tempfile.mkdtemp(prefix="ankusdrive_modal_gate_")
    try:
...
def _performance_contract_gate(comp_of_inst, links_by_inst, by_name):
    ...
    Every linked component is consulted for a declared `AD_Performance` contract and
    ...
    The gate never MEASURES; it reads the record. See ankusdrive/gates/performance.py for
...
def _mobility_gate(man):
    ...
    ankusdrive.mechanism over the manifest's `mechanism` block.
...
def _is_thumbnail(view):
    ...
    return bool(_prop_get(view, "AD_Thumbnail", False))
...
def _dim_parent_view(dim):
    name = str(_prop_get(dim, "AD_ParentView", "") or "")
    if name:
...
def _dim_text(dim):
    ...
    (AD_TrueValue) over TechDraw's projected raw value, so a dimension always
    reads the real geometry (validate-the-artifact)."""
    val = None
    if _prop_has(dim, "AD_TrueValue"):
        try:
            val = float(_prop_get(dim, "AD_TrueValue"))
        except Exception:
            val = None
    if val is None:
        try:
            ...
            return ""
    prefix = str(_prop_get(dim, "AD_Prefix", "") or "")
    if not prefix:
...
def _dim_tol_text(dim):
    """The tolerance suffix for a dimension's label, from stamped AD_TolPlus/
    AD_TolMinus (signed, plus>=minus): ' ±0.1' when symmetric, else
    ' +0.012/-0' (upper over lower). '' when no tolerance is attached."""
    if not (_prop_has(dim, "AD_TolPlus") and _prop_has(dim, "AD_TolMinus")):
        return ""
    try:
        plus = float(_prop_get(dim, "AD_TolPlus"))
        minus = float(_prop_get(dim, "AD_TolMinus"))
    except Exception:
        return ""
...
def _dim_is_leader(dim):
    ...
    return str(_prop_get(dim, "AD_Prefix", "") or "") in ("Ø", "R")
...
def _leader_layout(dim, cx, cy, idx, bbox=None):
    ...
    if len(pts) < 2:
        return None
    a, b = pts[0], pts[1]
    if str(_prop_get(dim, "AD_Prefix", "")) == "R":
...
# date / project supplied via set_title_block (stamped as AD_TitleBlock JSON).
...
def _page_title_fields(page):
    raw = str(_prop_get(page, "AD_TitleBlock", "") or "")
    if not raw:
        return None
    try:
...
def _stamp_title_block(page, fields):
    if not _prop_has(page, "AD_TitleBlock"):
        try:
            page.addProperty("App::PropertyString", "AD_TitleBlock",
                             "AnkusDrive", "title block fields (JSON)")
        except Exception:
            return
    try:
        _prop_set(page, "AD_TitleBlock", json.dumps(fields))
    except Exception:
        pass
...
def _title_block_svg(page, page_w, page_h):
    ...
    project) come from the AD_TitleBlock stamp. Returns '' if no stamp is present
    ...
    return "<g id=\"ankusdrive-titleblock\">\n" + "\n".join(seg) + "\n</g>"


# --- feature control frames (issue #232) --------------------------------------
# A GD&T callout is a page annotation carrying a AD_Gdt JSON stamp. It renders as a
...
def _gdt_spec(ann):
    """The AD_Gdt stamp on an annotation as a dict, or None."""
    raw = str(_prop_get(ann, "AD_Gdt", "") or "")
    if not raw:
        return None
    try:
        return json.loads(raw)
...
def _iter_placed_dims(page):
    ...
    from ankusdrive import drawing_gate
    views = _page_part_views(page)
...
# number lives on the FreeCAD object (AD_Balloon), so it survives save/reopen and a
...
def _balloon_of(obj):
    ...
    n = _prop_get(obj, "AD_Balloon", None)
    try:
...
def _stamp_balloon(obj, n):
    if not _prop_has(obj, "AD_Balloon"):
        try:
            obj.addProperty("App::PropertyInteger", "AD_Balloon", "AnkusDrive",
                            "inspection balloon number (issue #232)")
        except Exception:
            return
    try:
        _prop_set(obj, "AD_Balloon", int(n))
    except Exception:
        pass
...
def _balloon_to_svg(lay):
    ...
    return ('<g class="ankusdrive-balloon">\n'
...
def _compose_page_svg(page):
    ...
    if tb:
        parts.append(tb)
    overlay = '<g id="ankusdrive-overlay">\n' + "\n".join(parts) + "\n</g>\n"
    idx = base.rfind("</svg>")
    if idx == -1:
...
def _stamp_true_value(dim, value):
    if not _prop_has(dim, "AD_TrueValue"):
        try:
            dim.addProperty("App::PropertyFloat", "AD_TrueValue",
                            "AnkusDrive", "true 3D measurement, mm")
        except Exception:
            return
    try:
        _prop_set(dim, "AD_TrueValue", float(value))
    except Exception:
        pass


def _stamp_prefix(dim, prefix):
    if not _prop_has(dim, "AD_Prefix"):
        try:
            dim.addProperty("App::PropertyString", "AD_Prefix",
                            "AnkusDrive", "symbol prefixed to the dimension text")
        except Exception:
            return
    try:
        _prop_set(dim, "AD_Prefix", prefix)
    except Exception:
        pass


def _stamp_parent_view(dim, view):
    if not _prop_has(dim, "AD_ParentView"):
        try:
            dim.addProperty("App::PropertyString", "AD_ParentView",
                            "AnkusDrive", "name of the view this dimension annotates")
        except Exception:
            return
    try:
        _prop_set(dim, "AD_ParentView", view.Name)
    except Exception:
        pass
...
def _stamp_str_prop(obj, name, value):
    ...
    name = _prop_name(obj, name)      # keep updating a pre-rename DP_ stamp (#295)
    if not hasattr(obj, name):
        try:
            obj.addProperty("App::PropertyString", name, "AnkusDrive", name)
        except Exception:
            return
    try:
...
def _stamp_model_ref(dim, ref):
    ...
    — as JSON in AD_ModelRef. The manufacturability gate (drawing_gate) reads this to
    ...
    if not _prop_has(dim, "AD_ModelRef"):
        try:
            dim.addProperty("App::PropertyString", "AD_ModelRef",
                            "AnkusDrive", "model-space geometry this dim references (JSON)")
        except Exception:
            return
    try:
        _prop_set(dim, "AD_ModelRef", json.dumps(ref))
    except Exception:
        pass
...
def _stamp_tolerance(dim, plus, minus):
    ...
    for name, val in (("AD_TolPlus", plus), ("AD_TolMinus", minus)):
        if not hasattr(dim, name):
            try:
                dim.addProperty("App::PropertyFloat", name, "AnkusDrive",
...
def _resolve_tolerance(tol, basic):
    ...
    if "fit" in tol:
        from ankusdrive.analysis import tolerance as _T
        code = str(tol["fit"])
...
def _h_add_dimension(p):
    ...
    def _record(dim, view, true_value=None, model_ref=None):
        ...
        h = _register("dim", dim)
        try:
            shown = float(_prop_get(dim, "AD_TrueValue")) if _prop_has(dim, "AD_TrueValue") \
...
def _h_add_feature_note(p):
    ...
    ann.X = float(p.get("x", 20.0))
    ann.Y = float(p.get("y", 20.0))
    _stamp_str_prop(ann, "AD_FeatureNote", feature)
    _stamp_str_prop(ann, "AD_NoteKind", str(p.get("kind", "feature")))
    doc.recompute()
    h = _register("note", ann)
...
def _h_set_title_block(p):
    ...
    template is a bare sheet, so AnkusDrive composes its own bottom-right block on
...
# solid (AD_TrueValue): a green render is NOT a manufacturable drawing. These two
...
# accounting/geometry lives in ankusdrive.drawing_gate (FreeCAD-free, unit-tested);
...
def _dim_descriptors(page, datum_faces=None):
    ...
    its kind (Ø/R via AD_Prefix, else linear Type), its true value (AD_TrueValue,
    so it reads the real geometry), and the model-space geometry it references
    (AD_ModelRef → circle/span). Dims without a model ref still size-match by value.
    ...
    for dim in _page_dimensions(page):
        ...
        try:
            value = float(_prop_get(dim, "AD_TrueValue")) if _prop_has(dim, "AD_TrueValue") \
                else float(dim.getRawValue())
        except Exception:
            value = 0.0
        ...
        if _prop_has(dim, "AD_TolPlus") and _prop_has(dim, "AD_TolMinus"):
            try:
                d["plus"] = float(_prop_get(dim, "AD_TolPlus"))
                d["minus"] = float(_prop_get(dim, "AD_TolMinus"))
            except Exception:
                pass
        raw = str(_prop_get(dim, "AD_ModelRef", "") or "")
        if raw:
            try:
                ref = json.loads(raw)
...
def _page_feature_notes(page):
    ...
    AD_FeatureNote = the enumerated feature id it documents. The drawing_gate credits
    them as coverage for curved/periodic slots a dimension can't capture (#108)."""
    out = []
    for o in page.Views:
        fid = _prop_get(o, "AD_FeatureNote", None)
        if fid:
            out.append({"feature": str(fid),
                        "kind": str(_prop_get(o, "AD_NoteKind", "feature") or "feature")})
    return out
...
def _balloon_coverage(page):
    ...
    from ankusdrive import inspection
    try:
...
def _page_dim_envelope(page):
    """The bounding box [x0,y0,x1,y1] in page mm of everything AnkusDrive draws —
...
# lives in ankusdrive.inspection (FreeCAD-free, unit-tested). These shims only read
...
def _page_inspection_inputs(page):
    """(dims, gdt, notes) descriptors for ankusdrive.inspection, off the real page."""
...
def _h_add_gdt_callout(p):
    ...
    _stamp_str_prop(ann, "AD_Gdt", json.dumps(spec))
    doc.recompute()
...
def _h_balloon_drawing(p):
    ...
    (AD_Balloon), so re-running on an unchanged page reassigns nothing, adding a
...
def _stamp_thumbnail(view):
    if not _prop_has(view, "AD_Thumbnail"):
        try:
            view.addProperty("App::PropertyBool", "AD_Thumbnail", "AnkusDrive",
                             "isometric pictorial reference (not dimensioned/gated)")
        except Exception:
            return
    try:
        _prop_set(view, "AD_Thumbnail", True)
    except Exception:
        pass
...
#
# Unlike render_view (the host-side NumPy rasterizer in ankusdrive/render.py),
...
# binary. AnkusDrive locates the binary at call time and writes its path into the
...
# Mirrors ankusdrive/render.py's _VIEWS so render_view and render_photoreal frame a
...
def _renderer_exec_candidates(renderer, spec):
    ...
    ANKUSDRIVE_<RENDERER>_PATH env override -> path already set in FreeCAD prefs ->
    ...
    if env_path := _config.get(f"ANKUSDRIVE_{renderer.upper()}_PATH"):
...
def _resolve_renderer_exec(renderer):
    ...
    if spec is None:
        raise RuntimeError(
            f"renderer {renderer!r} is not wired in AnkusDrive yet (Phase 1 supports "
    ...
    raise RuntimeError(
        ...
        f"ANKUSDRIVE_{renderer.upper()}_PATH to its full path."
    )
...
def _h_render_photoreal(p):
    ...
    tmp = App.newDocument("ankusdrive_render")
    try:
...
# --- external-solver provisioning (ankusdrive.solvers) --------------------------
...
# live in ankusdrive/solvers.py (pure-Python, FreeCAD-free — so the degradation
...
def _require_solver(name):
    ...
    when it does not. See ankusdrive/solvers.py."""
...
def _h_solve_capabilities(p):
    ...
    solver side-effect-free (ANKUSDRIVE_<SOLVER>_PATH env -> PATH -> per-OS install
    dirs for binaries; importability for pip-wheel solvers) and executes nothing.

    A family's ``any_available`` means AnkusDrive can DRIVE it here, not merely that a
    binary resolved: a solver no AnkusDrive tool can build a case for (SU2 — every
...
def _shape_handle_to_obj(handle_or_name):
    """Accept either a AnkusDrive handle or a raw object name."""
...
def _h_random_vibration(p):
    ...
    PSD. See ankusdrive.analysis.vibration. Returns {rms_g, first_mode_hz,
    dominant_mode_hz, q, psd_band_hz, miles_grms_g, modes, rms_stress_mpa,
    three_sigma_stress_mpa, pass}."""
    from ankusdrive.analysis import vibration
...
def _h_beam_modal(p):
    ...
    ankusdrive.analysis.vibration. Returns {boundary, n_modes, frequencies_hz, beta_l,
...
def _h_fluid_props(p):
    from ankusdrive.analysis import fluids
...
def _h_bolted_joint_check(p):
    from ankusdrive.analysis import machine_elements as me
...
def _h_laminate_properties(p):
    from ankusdrive.analysis import laminate
...
def _h_fatigue_check(p):
    from ankusdrive.analysis import durability
...
def _h_h_estimate(p):
    ...
    ankusdrive.analysis.convection. Returns {geometry, mode, correlation, h_conv_w_m2k,
    ...
    from ankusdrive.analysis import convection
...
def _h_acoustic_screen(p):
    ...
    per kind. See ankusdrive.analysis.acoustics. Returns {kind, c_m_s, fidelity,
    band_pct, band_db, valid_range_ok, warnings, escalate_to} + per-kind fields."""
    from ankusdrive.analysis import acoustics
...
def _h_plate_check(p):
    ...
    exact within thin-plate theory, limits flagged). See ankusdrive.analysis.plates.
    ...
    fidelity, band_pct, valid_range_ok, warnings, escalate_to}."""
    from ankusdrive.analysis import plates
...
def _h_beam_buckling(p):
    ...
    fem_buckling eigen-solve is gated against. See ankusdrive.analysis.buckling.
    ...
    from ankusdrive.analysis import buckling
...
def _h_plastic_collapse(p):
    ...
    ankusdrive.analysis.nonlinear. Returns {support, S_elastic_mm3, Z_plastic_mm3,
    ...
    from ankusdrive.analysis import nonlinear
...
def _h_elastica_deflection(p):
    ...
    ankusdrive.analysis.nonlinear. Returns {alpha, tip_slope_deg, tip_disp_mm,
...
def _h_hertz_contact(p):
    ...
    ankusdrive.analysis.nonlinear. Returns {e_star_mpa, effective_radius_mm,
...
def _h_fsi_plate_deflection(p):
    ...
    ankusdrive.analysis.fsi. Returns {support, pressure_pa, line_load_n_per_mm,
    ...
    escalate_to}."""
    from ankusdrive.analysis import fsi
...
def _h_fsi_interface_balance(p):
    ...
    relative residual is the conservation error. See ankusdrive.analysis.fsi.
...
def _h_fsi_channel_pressure(p):
    ...
    Reynolds number / laminar regime. See ankusdrive.analysis.fsi. Returns
...
def _h_waveguide_cutoff(p):
    ...
    ankusdrive.analysis.em_fullwave. Returns {mode, m, n, a_mm, b_mm, eps_r,
    ...
    from ankusdrive.analysis import em_fullwave
...
def _h_dipole_resonance(p):
    ...
    on the wire-diameter spread. See ankusdrive.analysis.em_fullwave. Returns
...
def _h_monopole_sphere(p):
    ...
    ankusdrive.analysis.acoustics_bem. Returns {a_m, freq_hz, k_per_m, ka, u_amp, rho,
    ...
    valid_range_ok, warnings, escalate_to}."""
    from ankusdrive.analysis import acoustics_bem
...
def _h_rigid_sphere_scattering(p):
    ...
    backscatter. See ankusdrive.analysis.acoustics_bem. Returns {ka, theta_deg, a_m,
...
def _h_molding_screen(p):
    ...
    reach (±30% chart). See ankusdrive.analysis.molding. Returns {material,
...
def _h_moldability_screen(p):
    ...
    mold-shrinkage / crystallinity fields. See ankusdrive.analysis.molding. Returns
...
def _h_drop_impact(p):
    ...
    ankusdrive.analysis.impact. Returns {drop_height_mm, impact_velocity_m_s, pulse,
...
def _h_thermal_transient_1d(p):
    ...
    answer the lumped screen only approximates. See ankusdrive.analysis.thermal. Returns
...
def _h_cfd_pipe_flow(p):
    ...
    against. See ankusdrive.analysis.cfd. Returns {reynolds, regime, velocity_m_s,
...
def _h_cfd_body_drag(p):
    ...
    Returns the ankusdrive.analysis.cfd verdict: {cd, drag_force_n, frontal_area_m2,
...
def _h_dfm_check(p):
    ...
    * **live handle (v2 Shape wiring)** — pass a `handle`/`model` and the per-face
    ...
    block to screen bends on a part AnkusDrive did not model. Either way there is one
...
def _h_cost_estimate(p):
    from ankusdrive.analysis import cost
...
def _h_tolerance_cost_check(p):
    ...
    default cnc). See ankusdrive.analysis.tolerance_cost. Returns {process, links,
    n_links, total_cost_index, mean_cost_index, flagged, pass, fidelity, band_pct,
    basis, escalate_to='suggest_loosening'}."""
    from ankusdrive.analysis import tolerance_cost
    p = dict(p)
...
def _h_suggest_loosening(p):
    ...
    ankusdrive.analysis.tolerance_cost. Returns {ok, steps, stopped, chain,
...
def _h_cnc_machinability_check(p):
    ...
    min_wall_mm (0.8), max_setups (3). See ankusdrive.analysis.machining. Returns
    ...
    limitations, n_faces}."""
    from ankusdrive.analysis import machining
...
def _h_slice_estimate(p):
    from ankusdrive.analysis import slicing
...
def _h_moldability_check(p):
    ...
    moldability screen (ankusdrive.analysis.molding): the wall-thickness quality
...
def _h_optics_raytrace(p):
    ...
    if not info["ok"]:
        return info
    from ankusdrive.analysis import optics
...
def _h_optics_lens_design(p):
    ...
    if not info["ok"]:
        return info
    from ankusdrive.analysis import optics_design
...
def _run_optics_gpl(problem, python_exe, timeout=120):
    ...
    ankusdrive/optics_gpl_runner.py (a standalone script that imports no ankusdrive code)
    and return its JSON result. AnkusDrive never imports KrakenOS in-process; this
...
# Exactly the KrakenOS arrangement: openEMS/CSXCAD are GPL-3.0, so AnkusDrive never
...
def _run_em_fullwave_gpl(problem, python_exe, timeout=600):
    ...
    ankusdrive/em_fullwave_gpl_runner.py (a standalone script that imports no ankusdrive
    code) and return its JSON result. AnkusDrive never imports openEMS in-process;
...
# preCICE is LGPL and both heavy solvers run as their own subprocesses — AnkusDrive
...
# ankusdrive/dem_gpl_runner.py, exchanging sentinel-JSON over stdin/stdout, exactly
# the arm's-length isolation used for KrakenOS/openEMS. AnkusDrive never imports
...
def _yade_exec():
    ...
    ANKUSDRIVE_YADE override → ANKUSDRIVE_YADE_PATH (the solver-registry env) →
...
def _run_dem_gpl(problem, yade_exe, timeout=600):
    ...
    ankusdrive/dem_gpl_runner.py (a standalone script that imports no ankusdrive code)
    and return its JSON result. AnkusDrive never imports YADE in-process; this
...
def _h_granular_oracle(p):
    ...
    See ankusdrive.analysis.granular."""
    from ankusdrive.analysis import granular
...
# Bempp is MIT — the SAME permissive footing as AnkusDrive — so the subprocess here
# is NOT a license boundary. It is a DEPENDENCY-CLASH boundary: Bempp needs
# meshio>=4 (cells_dict), but AnkusDrive's shared venv pins meshio==3.0 for solidspy
# (ankusdrive/analysis/topology.py). Rather than break topology optimisation, Bempp
# lives in a DEDICATED venv (.venv-bempp) and is invoked OUT-OF-PROCESS via
# ankusdrive/bempp_runner.py (sentinel-delimited JSON over a pipe), resolved exactly
...
def _run_bempp(problem, python_exe, timeout=900):
    """Invoke the Bempp BEM engine OUT-OF-PROCESS via ankusdrive/bempp_runner.py (a
    standalone script that imports no ankusdrive code) and return its JSON result.
    AnkusDrive never imports bempp_cl in-process; this fork/exec + pipe-IPC keeps the
...
def _h_mechanism_kinematics(p):
    ...
    See ankusdrive.analysis.kinematics."""
    from ankusdrive.analysis import kinematics as kin
...
def _mbd_to_si_spec(links, drivers, obstacles=None, base=None, gears=None):
    ...
    the SI spec ankusdrive.analysis.mbd.run_mbd expects. Pure unit math, FreeCAD-free —
...
def _molding_warpage_submit(p, info):
    ...
    from ankusdrive import jobs
    from ankusdrive.analysis import warpage as _warp
...
def _h_harmonic_response(p):
    ...
    against. See ankusdrive.analysis.vibration. Returns {natural_frequency_hz,
...
def _h_cfd_internal_flow_submit(p):
    ...
    Four ways to drive it:
      * **Build the native SU2 plane-channel case** — pass `channel_height_mm` (plus
        ...
        `nx`/`ny`, `max_iterations`). The ONLY built-in case AnkusDrive builds for SU2,
...
def _cfd_channel_su2_submit(p):
    ...
    The only built-in CFD case AnkusDrive can build for SU2, and the reason the cfd
    ...
    from x = 0 (see ankusdrive/analysis/su2_case.py for why that matters).
    ...
    from ankusdrive import jobs, solvers
    from ankusdrive.analysis import cfd as _cfd
    from ankusdrive.analysis import su2_case as _su2
...
def _cfd_external_body_submit(p):
    ...
    from ankusdrive.analysis import cfd as _cfd_gate
    ...
    def _work():
        from ankusdrive.analysis import meshbridge as _mb2
        from ankusdrive.analysis import openfoam as _of
...
def _h_grid_convergence(p):
    ...
    ankusdrive.analysis.verification.grid_convergence: fits the observed order of
    ...
    fidelity, warnings}."""
    from ankusdrive.analysis import verification
...
def _h_cfd_mesh_independence_submit(p):
    ...
    def _work():
        from ankusdrive.analysis import meshbridge as _mb2
        from ankusdrive.analysis import openfoam as _of
        from ankusdrive.analysis import verification as _ver
...
# ankusdrive/analysis/molding_fill.fill_gate).
...
def _molding_openinjmoldsim_build_and_run(p, of_bin):
    ...
    try:
        from ankusdrive.analysis import molding_fill as _mf0
...
# --- generic async jobs (ankusdrive.jobs) ---------------------------------------
...
# (see ankusdrive/jobs.py) — for an FEM/CFD solve, background only the solver
...
# schema, typed validation, and the reference recipes all live in ankusdrive/recipes.py
...
def _h_recipe_list(p):
    ...
    from ankusdrive import recipes as _rcp
...
# the owned, pure-Python module ankusdrive/items.py (no FreeCAD); these handlers are
...
# pure-Python module ankusdrive/project.py (no FreeCAD); these handlers are thin
...
def _h_scaffold_project(p):
    ...
    master, dirs}."""
    from ankusdrive import project as _proj
...
# ankusdrive/families.py — table parsing, the column split, the recipe door, and the
...
from ankusdrive import families as _families_register  # noqa: E402,F401  (registers catalog recipes)
...
def _h_family_validate(p):
    ...
    from ankusdrive import families as _fam
...
# lives in the owned, FreeCAD-free module ankusdrive/gates/substitutability.py; this
...
def _h_substitutability_check(p):
    ...
    performance?, ...}."""
    from ankusdrive.gates import substitutability as _subst
...
# C2). All logic lives in the owned, pure-Python module ankusdrive/lifecycle.py (no
...
def _h_lifecycle_editable(p):
    ...
    {ok, editable, state}."""
    from ankusdrive import items as _items
    from ankusdrive import lifecycle as _lc
    reg = _items.load_registry(p["registry"])
...
# AnkusDrive's PowerCopy/UDF (RFC docs/DESIGN_HIERARCHY.md §2 Theme B / B2). The
# registry, ref/scalar declaration, typed validation, and the reference templates
# all live in ankusdrive/feature_templates.py (a pure-Python module that touches
...
def _h_feature_list(p):
    ...
    from ankusdrive import feature_templates as _ft
...
# owned, pure-Python module ankusdrive/change.py (no FreeCAD); these handlers are
...
def _h_where_used(p):
    ...
    from ankusdrive import change as _chg
...
# FreeCAD-free ankusdrive/release.py and is unit-tested without a CAD kernel. This
...
def _h_release_package(p):
    ...
    from ankusdrive import inspection as _insp
    from ankusdrive import items as _items
    from ankusdrive import release as _rel
...
# development live in the FreeCAD-free `ankusdrive.sheetmetal` core instead. These
...
def _sheet_build(steps):
    """Turn `ankusdrive.sheetmetal` build recipes into one fused OCC shape.
...
def _sheet_locate(handle, ref, width_mm=None, offset_mm=0.0):
    ...
    from ankusdrive import sheetmetal as sm
...
def _main():
    from ankusdrive import mainthread

"""
AnkusDrive MCP server — exposes the worker handlers as MCP tools over stdio.
...
The pool is capped (ANKUSDRIVE_MAX_WORKSPACES, default 4) and idle-reaped
(ANKUSDRIVE_WORKSPACE_IDLE_S, default 900s) so abandoned workspaces don't leak
freecadcmd processes. See `use_workspace` / `list_workspaces` / `close_workspace`
and docs/MULTI_AGENT.md.

Run:   .venv/bin/python3 -m ankusdrive mcp
"""
import atexit
import base64
import os
...
_MAX_WORKSPACES = max(1, int(os.environ.get("ANKUSDRIVE_MAX_WORKSPACES", "4")))
_IDLE_REAP_S = float(os.environ.get("ANKUSDRIVE_WORKSPACE_IDLE_S", "900"))
...
def use_workspace(name: str) -> dict:
    ...
    The pool is capped (ANKUSDRIVE_MAX_WORKSPACES, default 4) and idle workspaces are
    reaped (ANKUSDRIVE_WORKSPACE_IDLE_S, default 900s); claiming a workspace beyond a
...
def register_handle(object: str, prefix: str = "manual") -> dict:
    """Register an existing FreeCAD object into the AnkusDrive handle table.
...
def catalog_search(family: str | None = None, standard: str | None = None,
    ...
    This one tells you whether it is a thing you can buy. Nothing else in AnkusDrive
...
def solve_capabilities() -> dict:
    ...
    ANKUSDRIVE_<SOLVER>_PATH env -> PATH -> per-OS install dirs; a pip-wheel solver by
...
def diagnose_setup() -> str:
    """Diagnose this AnkusDrive install and guide the user through finishing it."""
    return (
        "Diagnose my AnkusDrive setup. Call the `setup_status` tool (leave "
...
@mcp.resource("ankusdrive://setup")
def setup_resource() -> str:
    """The current setup/health report (the human-readable `ankusdrive doctor`
...
# AnkusDrive's permissive code; absent YADE, the *_submit tools degrade to
...
def async_demo_submit(duration_s: float = 0.5, value: float = 1.0) -> dict:
    ...
    FEM/CFD solve plugs into the same facility — see ankusdrive/jobs.py.) Returns
...
def items_validate(registry: str) -> dict:
    ...
    Checks the schema stamp ("ankusdrive.items/1"), each item record's shape, that
...
# Append-only exposure of ankusdrive/lifecycle.py (built on the C1 item model).
...
def project_validate(project: str) -> dict:
    ...
    ("ankusdrive.project/1"), the naming convention on the project name, the
...
# Append-only exposure of ankusdrive/change.py — the change-control layer over the
